...
// Returns { match, unknownFields }. Any unknown referenced field makes match false.
...
export function evaluateRisk(risk, ctx) {
  ...
  if (risk.activeWhen) {
    ...
    // Unknown inputs: keep the risk in scope (conservative) and flag it
...
export function scoreProject(profile, now = new Date()) {
  ...
  // Data gaps: unknown field -> active risks that depend on it
  const dataGaps = {};
